...
export function WorkspaceEditor({
  ...
  return (
    <div className="w-[42%] flex flex-col h-full overflow-hidden bg-[#1a0f0f] border-r border-[#3d1f1f] shrink-0">
      <div className="flex h-11 items-center justify-between px-4 border-b border-[#3d1f1f] bg-[#2a1515]/50 shrink-0">
        ...
        {!isSubmitted && (
          <div className="flex gap-2">
            <Button size="sm" variant="outline" onClick={onRun} disabled={isRunning || isSubmitting}
              className="bg-transparent border-[#3d1f1f] hover:bg-[#3d1f1f] text-foreground h-8">
      ...
      <div className="flex-1 min-h-[250px] border-b border-[#3d1f1f]">
        <Editor
          height="100%"
      ...
      <div className="h-56 flex flex-col overflow-hidden bg-[#2a1515]/60 shrink-0">
        <div className="flex h-8 items-center justify-between px-3 border-b border-[#3d1f1f] bg-[#2a1515]/80 shrink-0">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-[45%] overflow-y-auto p-3 bg-[#1a0f0f]/60 font-mono text-[10px] text-muted-foreground whitespace-pre-wrap select-text">
...
function TestResultsPanel({ results, isSubmitted }: { results: TestRunResult[] | null; isSubmitted: boolean }) {
  return (
    <div className="w-[55%] border-r border-[#3d1f1f] overflow-y-auto p-2.5 space-y-1.5">
        ...
        results.map((tc, index) => {
          ...
          return (
            <div key={tc.testCaseId || index}
              ...
              {tc.input && (
                <div className="text-[9px] font-mono text-muted-foreground">Input: <span className="text-muted-foreground">{tc.input}</span></div>
              )}
              <div className="text-[9px] font-mono text-muted-foreground">Expected: <span className="text-muted-foreground">{tc.expectedOutput}</span></div>
              {!tc.passed && (
